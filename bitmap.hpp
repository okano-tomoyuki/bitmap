...
private:
    ...
    std::vector<std::vector<int>> data_;            /**!< value of pixel */
...
public:
     ...
     */
    Bitmap& set_data(std::vector<std::vector<int>>&& data, const std::vector<uint32_t>& color_pallet, const enum Format& format=BIT4) &
    {
        if(format!=BIT4)
     ...
     * @return Bitmap& データセット後の自身のインスタンスを返却する。
     */
    Bitmap& set_data(std::vector<std::vector<int>>&& data, const enum ColorTheme& color_theme=DAY, const enum Format& format=BIT4) &
    {
        if(format!=BIT4)
            throw std::runtime_error("only 4bit format is implemented.");
        ...
        if(format!=BIT24)
        {
            ...
            color_pallet_.resize(pallet_size/4);
            if(color_theme==DAY)
                color_pallet_ = DAY_COLOR_4BIT;
            else if(color_theme==NIGHT)
                color_pallet_ = NIGHT_COLOR_4BIT;
            else if(color_theme==GRAYSCALE)
                color_pallet_ = GRAYSCALE_COLOR_4BIT;
            else
    ...
    Bitmap& rotate(const enum Rotate& rotate=PLUS90) &
    {
        ...
        std::vector<std::vector<int>> tmp(header_.height, std::vector<int>(header_.width, 0));
        if(rotate==PLUS90) {
            for(int i=0; i<header_.height; i++)
                for(int j=0; j<header_.width; j++)
                    tmp.at(i).at(j) = data_.at(header_.width-1-j).at(header_.height-1-i);
        } else if(rotate==MINUS90) {
            for(int i=0; i<header_.height; i++)
                for(int j=0; j<header_.width; j++)
                    tmp.at(i).at(j) = data_.at(j).at(i);
        } else {
    ...
    void save(const std::string& file_path) const
    {
        ...
        int width_in_bytes = (header_.width * header_.bpp + 31)/32 * 4;
        std::vector<u_char> row_data(width_in_bytes, 0);
        ...
        for(int i=0; i<header_.height; i++)
        {
            ...
            {
                if(j%2==0)
                    row_data.at(j/2) = (data_.at(i).at(j) << 4);
                else
                    row_data.at(j/2) += data_.at(i).at(j);
            }
            std::fwrite(row_data.data(), sizeof(u_char), width_in_bytes, file_ptr);
        }
